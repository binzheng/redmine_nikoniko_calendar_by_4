...
class NikonikoCalendarController < ApplicationController
  ...
  def history
    ...
    logger.debug("Hello, world!001")
    logger.debug(@calendar)
    logger.debug(User)
    logger.debug(@calendar.startdt.to_s)
    logger.debug(User.current.id)
    # get user's nikoniko
    @nikoniko_history = {};
    nikoniko_history_tmp = NikonikoHistory.where(
